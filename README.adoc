...
image::modules.png[title="CloudSim Plus Modules", scaledwidth="75%"]
...
image::package-structure-reduced-cloudforum.png[title="CloudSim Plus Simplified Package Diagram", scaledwidth="75%"]
...
image::reduced-main-classes-cloudforum.png[title="CloudSim Plus Simplified Class Diagram", scaledwidth="75%"]
...
- `onSimulationPausedListener`: get notifications when the simulation is paused. Such a Listener can be used, for instance, to collect partial simulation data after the simulation is intentionally paused at a given time. Scientifically valid simulation results depend on, among other factors, statistical data obtained from multiple simulation runs. When presenting such results one should consider a defined confidence interval that shows with a percentage of confidence, the standard deviation in which the results rely on. One of the methods to reduce the confidence interval is the https://en.wikipedia.org/wiki/Replication_(statistics):[replication method], in which the simulation is reproduced several times using different seeds. Using this method, to reduce the confidence interval by half requires to quadruple the number of simulation runs. Other methods such as the https://en.wikipedia.org/wiki/Regenerative_process:[regeneration process] is one of those that enables reducing such an interval, when it is realized that at some point in time, some studied random variable returns to its initial state. Pausing the simulation at this point enables collecting the required data when such an event happens.
...
CloudSim Plus was comprehensive re-engineered to create relationships among classes, enabling chained calls such as `cloudlet.getVm().getHost().getDatacenter()`. As the presented <<fig:class-diagram>> shows, an object,such as a `Cloudlet`, defines relationships to keep a reference to each object it is related to. Accordingly, the line of code presented above provides a direct way to know what `Vm` a `Cloudlet` is running or will run, what `Host` such a `Vm` is placed into and finally what is the `Datacenter` where such a `Host` is settled down. The https://en.wikipedia.org/wiki/Null_Object_pattern:[Null Object Design Pattern] was also implemented to avoid the infamous `NullPointerException` when making such a chained call.
...
=== As it is usual to extend framework classes to provide some specific behaviors for your simulations, you will find a totally refactored code that follows clean code programming, 
