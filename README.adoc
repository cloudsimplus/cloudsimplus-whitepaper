...
image::package-structure-reduced-cloudforum.png[title="CloudSim Plus Simplified Package Diagram"]
...
image::reduced-main-classes-cloudforum.png[title="CloudSim Plus Simplified Class Diagram"]
...
- `onSimulationPausedListener`: get notifications when the simulation is paused. Such a Listener can be used, for instance, to collect partial simulation data after the simulation is intentionally paused at a given time. Scientifically valid simulation results depend on, among other factors, statistical data obtained from multiple simulation runs. Presentation of such results should take into account a defined confidence interval that shows with a percentage of confidence, the standard deviation in which the results rely on. One of the methods to reduce the confidence interval is the replication method, in which the simulation is reproduced several times using different seeds. Using this method, reducing the confidence interval by half requires to quadruple the number of simulation runs. Other methods such as the regeneration process is one of those that enables reducing such a interval when it is realized that at some point in time, some studied random variable returns to its initial state. Pausing the simulation at this point enables collecting the required data when such an event happens.
